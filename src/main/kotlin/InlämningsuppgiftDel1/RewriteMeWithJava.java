...
public class RewriteMeWithJava {

    public Database database = new Database();
    public List<Question> questions = database.getList();
    ...
    public int getAmountOfQuestionsForACertainCategory(Category category){
        return (int) questions.stream().filter(c -> c.getCategory() == category).count();
    }
    ...
    public List<String> getAllQuestionStringsBelongingACategory(Category category){
        return questions.stream().filter(c -> c.getCategory() == category).map(Question::getQuestion).collect(Collectors.toList());
    }
    ...
    public List<String> getAllAnswerOptionsDistinct(){
        List<String> list = questions.stream().map(Question::getAnswers).flatMap(List::stream).collect(Collectors.toList());
    ...
    {
        List<String> list = questions.stream().filter(q -> q.getCategory() == c).map(a -> a.getAnswers())
    ...
    public static void main(String[] args){
        RewriteMeWithJava uppg4 = new RewriteMeWithJava();
    }
}